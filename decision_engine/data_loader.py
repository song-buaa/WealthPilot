...
class LoadedData:
    ...
    target_position: Optional[PositionInfo] # 被决策标的当前持仓（可为 None）
...
def load(asset_name: Optional[str], pid: int = default_portfolio_id) -> LoadedData:
    ...
    try:
        ...
        if asset_name:
            target_position = _find_position(positions, asset_name)
...
def _find_position(positions: list[PositionInfo], asset_name: str) -> Optional[PositionInfo]:
    """按名称或 ticker 模糊匹配持仓。
    ...
    for p in positions:
        ...
        name_match = (name_lower in p_name) or (p_name in name_lower and len(p_name) >= 2)
        # ticker 匹配要求 ticker 非空，且至少 2 个字符，避免空字符串误匹配
        ticker_match = (
            len(p_ticker) >= 2

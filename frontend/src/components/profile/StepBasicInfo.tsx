/**
 * Step 2 — 基础信息
 * 9 个字段全部用 <select>，必填：total_assets / income_stability / family_status
 */
...
const FIELDS: Field[] = [
  { key: 'total_assets',          label: '总资产规模',    required: true,  options: ['<50万','50-200万','200-500万','>500万'] },
  ...
  { key: 'family_status',         label: '家庭状态',      required: true,  options: ['单身','已婚无子','已婚有子','退休'] },
...
export default function StepBasicInfo({ data, onChange, onNext, onPrev }: Props) {
  const requiredFilled = ['total_assets','income_stability','family_status'].every(

...
| 投资纪律 | `app_pages/discipline.py` | ✅ 已实现 | 账户风险仪表盘 + 纪律手册速查；交易前评估入口已迁移至「投资决策」 |
| 投研观点 | `app_pages/research.py` | ✅ 已实现 | v1.9.0 新增，四 Tab 页面 |
| 投资决策 | `app_pages/strategy.py` | ✅ 已实现 | 统一的交易前决策入口，含意图解析、规则校验、信号分析、AI 建议 |

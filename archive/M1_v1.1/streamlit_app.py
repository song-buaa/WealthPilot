...
@st.cache_data(ttl=5)
def _get_position_count(pid: int) -> int:
...
# 加载投资组合信息
position_count = _get_position_count(portfolio_id)
...
def page_overview():
    ...
    # 在函数内部获取 portfolio，避免依赖模块级全局变量
    _session = get_session()
    try:
        portfolio = _session.query(Portfolio).filter_by(id=portfolio_id).first()
    finally:
        _session.close()
